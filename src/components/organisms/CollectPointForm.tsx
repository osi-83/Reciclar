...
import type { LocalColeta } from '../../types/index';
...
export function CollectPointForm({ usuarioId, onSalvar, localEditavel }: Props) {
    ...
    useEffect(() => {
        if (localEditavel) {
            ...
            setResiduos(localEditavel.residuos || []);
            setErroCep('');
        } else {
            setNome('');
            setDescricao('');
    ...
    const handleSalvar = () => {
        if (!nome || !descricao || !cep || !logradouro || !bairro || !cidade || residuos.length === 0) {
            alert('Por favor, preencha todos os campos obrigatórios e selecione pelo menos um resíduo.');
            return;
        }
    ...
    return (
        <form onSubmit={(e) => { e.preventDefault(); handleSalvar(); }} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', padding: '2rem', border: '1px solid #eee', borderRadius: '8px' }}>
            ...
            <div>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    <input
                        ...
                        required
                    />
                    <button  className= "botao" type="button" onClick={handleBuscarCep} style={{ padding: '0.5rem 1rem' }}>Buscar CEP</button>
                </div>
                {erroCep && <p style={{ color: 'red', fontSize: '0.8rem', marginTop: '0.2rem' }}>{erroCep}</p>}
            </div>
